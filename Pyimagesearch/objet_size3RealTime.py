...
cv2.createTrackbar("Min Contour Area", "Image", 200, 1000, on_trackbar)
...
while True:
    ...
    min_contour_area = cv2.getTrackbarPos("Min Contour Area", "Image")
    ...
    for c in cnts:
        ...
        # If the pixels per metric has not been initialized, compute it based on a reference object size
        if pixelsPerMetric is None:
            pixelsPerMetric = dB / 0.955  # Use a reference object width in inches
        ...
        cv2.putText(orig, "{:.1f} in".format(dimA), (int(tltrX - 15), int(tltrY - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.65,
                    (255, 255, 255), 2)
        cv2.putText(orig, "{:.1f} in".format(dimB), (int(tlblX - 15), int(tlblY - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.65,
                    (255, 255, 255), 2)
